...
public class TransferLadle_CarbonBlackSteel extends BlockTerra
{
...
public void onBlockPlacedBy(World w_, int x_, int y_, int z_, EntityLivingBase player, ItemStack is_)
{
	...
	if (!player.isSneaking() && w_.isAirBlock(x_+xdir, y_, z_+zdir))
	{
		...
		if (w_.isAirBlock(x_, y_+1, z_+zdir)) 
			w_.setBlock(x_, y_+1, z_+zdir, Blocks.fire, 0, 2);
	}
}
    ...
    public void registerBlockIcons(IIconRegister register)
    {
        ...
        _icontop = register.registerIcon(baseName + "TransferLadle_Top_CarbonBlackSteel");
    }
