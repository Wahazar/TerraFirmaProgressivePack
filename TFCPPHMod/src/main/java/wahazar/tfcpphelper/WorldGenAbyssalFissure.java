...
public class WorldGenAbyssalFissure implements IWorldGenerator
{
	private Random rand;
	...
	//Basic constructor
	public WorldGenAbyssalFissure(Block b)
	{
		fillBlock = b;
	}

	public WorldGenAbyssalFissure(Block b, int minTunnel, boolean s, int rare)
	{
		this(b);
	...
	public WorldGenAbyssalFissure setSeed(int i)
	{
		//seed = i;
		return this;
	}

	public WorldGenAbyssalFissure setUnderground(boolean i, int d)
	{
		underground = i;
		depth = d;
		return this;
			...
			IChunkProvider chunkGenerator, IChunkProvider chunkProvider) 
	{
		...
		if(rarity <= 0 || rand.nextInt(rarity) != 0 || biome != TFCBiome.DEEP_OCEAN)
			return;
	...
	{
		//additional vein minerals
		Block stibnite = GameRegistry.findBlock("ihl", "oreStibnite");
		if (stibnite==null)
			stibnite=Blocks.obsidian;
		Block fluorite = GameRegistry.findBlock("customitems", "fluorite");
		if (fluorite==null)
			fluorite=Blocks.glowstone;
		Block cinnabar = GameRegistry.findBlock("ihl", "oreCinnabar");
		if (cinnabar==null)
			cinnabar=TFCPPBlocks.blockSlag;
		Block prismarine = GameRegistry.findBlock("customitems", "prismarine");
		if (prismarine==null)
			prismarine=Blocks.sponge;
		Block salmiac = GameRegistry.findBlock("customitems", "salmiac_crystals");
		if (salmiac==null)
		...
		while(yCoord > minTunnel)
		{
			...
			if(fillBlock != null && world.getBlock(xCoord+1, yCoord, zCoord).getMaterial() != fillBlock.getMaterial())
				if ((fillBlock.getMaterial() == Material.water) && (addOre==0))
					world.setBlock(xCoord+1, yCoord, zCoord, stibnite, 0, 2);
				else if ((fillBlock.getMaterial() == Material.water) && (addOre==1))
					world.setBlock(xCoord+1, yCoord, zCoord, fluorite, 0, 2);			
				else
					world.setBlock(xCoord+1, yCoord, zCoord, rockLayer.block, rockLayer.meta, 2);
			if(fillBlock != null && world.getBlock(xCoord-1, yCoord, zCoord).getMaterial() != fillBlock.getMaterial())
				if ((fillBlock.getMaterial() == Material.water) && (addOre==2))
					world.setBlock(xCoord-1, yCoord, zCoord, fluorite, 0, 2);
				else if ((fillBlock.getMaterial() == Material.water) && (addOre==3))
